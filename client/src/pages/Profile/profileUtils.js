...
export const getStatusColors = (status) => {
  // Handle both human-readable format and enum format from backend
  const statusUpper = status?.toUpperCase ? status.toUpperCase() : status;
...
export const getStatusLabel = (status) => {
  ...
  // Format the status if it's in UPPERCASE_FORMAT
  if (status && typeof status === 'string' && status === status.toUpperCase()) {
